...
const SENSOR_RESPONSES = {
  ...
  FULL_DIAGNOSTIC: async () => {
    ...
    return {
      output: issues === 0 ? 'Full diagnostic complete — all 8 sensors nominal' : `Diagnostic complete — ${issues} sensor(s) require attention`,
...
export const SENSOR_REGISTRY = [
  { id: 'LIDAR', label: 'LIDAR', value: '—', status: 'OK', statusType: 'ok' },
  { id: 'BARO', label: 'BARO', value: '—', status: 'OK', statusType: 'ok' },
  { id: 'IMU', label: 'IMU', value: '±0.02°', status: 'OK', statusType: 'ok' },
  { id: 'GPS', label: 'GPS', value: '—', status: 'INIT', statusType: 'warn' },
  { id: 'CAMERA', label: 'CAMERA', value: '4K/30', status: 'OK', statusType: 'ok' },
  { id: 'OBSTACLE', label: 'OBSTACLE', value: 'CLEAR', status: 'OK', statusType: 'ok' },
  { id: 'TEMP', label: 'THERMAL', value: '—', status: 'INIT', statusType: 'ok' },
  { id: 'PAYLOAD', label: 'PAYLOAD', value: '—', status: 'INIT', statusType: 'ok' },
]

export function seedSensorsFromTelemetry(registry, telemetry) {
  return registry.map(s => {
    switch (s.id) {
      ...
      case 'GPS': {
        const lat = telemetry.coords?.lat ?? '—'
        const lon = telemetry.coords?.lon ?? '—'
        return {
          ...s,
          value: `${lat}, ${lon}`,
          status: 'OK',
